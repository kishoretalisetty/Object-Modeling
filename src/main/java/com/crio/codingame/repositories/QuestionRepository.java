...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        List<Question> qList=questionMap.entrySet().stream().map(v->v.getValue()).collect(Collectors.toList());
     return qList;
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> qList=findAll();
        if(level==null)return qList;
        List<Question> qListWithLevelWise=qList.stream().filter(q->q.getLevel().equals(level)).
